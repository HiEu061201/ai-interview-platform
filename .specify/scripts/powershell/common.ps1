...
function Test-FeatureJsonMatchesFeatureDir {
    ...
    [bool]$onWindows = ($null -ne $IsWindows) ? $IsWindows : $true
...
<#
.SYNOPSIS
Parses a preset.yml manifest for strategy and file path using Python.
#>
function Invoke-YamlManifestStrategyParser {
    param($PyCmd, $Manifest, $TemplateName)
    $strategy = 'replace'
    $filePath = ''
    $yamlMissing = $false
...
"@ $Manifest $TemplateName 2>$pyStderrFile
        ...
        if ($stratResult) {
            ...
            if ($parts.Count -gt 1 -and $parts[1]) { $filePath = $parts[1] }
        }
    ...
    return @{ Strategy = $strategy; FilePath = $filePath; YamlMissing = $yamlMissing }
}

<#
.SYNOPSIS
Gets the template layers from presets, resolving strategy metadata.
#>
function Get-PresetTemplateLayers {
    param(
        [string]$TemplateName,
        [string]$RepoRoot,
        [string]$PresetsDir,
        [string[]]$SortedPresets
    )
    ...
    foreach ($presetId in $SortedPresets) {
        ...
        if ((Test-Path $manifest) -and $pyCmd) {
            $stratResult = Invoke-YamlManifestStrategyParser -PyCmd $pyCmd -Manifest $manifest -TemplateName $TemplateName
            if ($stratResult.YamlMissing -and -not $yamlWarned) {
                Write-Warning "PyYAML not available; composition strategies may be ignored"
                $yamlWarned = $true
            }
            $strategy = $stratResult.Strategy
            $manifestFilePath = $stratResult.FilePath
        }

        if ($manifestFilePath -and ([System.IO.Path]::IsPathRooted($manifestFilePath) -or $manifestFilePath -match '\.\.[\\/]')) {
            $manifestFilePath = ''
        }
    ...
    return @{ Paths = $layerPaths; Strategies = $layerStrategies }
}

<#
.SYNOPSIS
Composes template layers based on their strategies.
#>
function Compose-TemplateLayers {
    param($LayerPaths, $LayerStrategies)
...
function Resolve-TemplateContent {
    ...
    if (Test-Path $presetsDir) {
        ...
        if ($sortedPresets.Count -gt 0) {
            $presetLayers = Get-PresetTemplateLayers -TemplateName $TemplateName -RepoRoot $RepoRoot -PresetsDir $presetsDir -SortedPresets $sortedPresets
            $layerPaths += $presetLayers.Paths
            $layerStrategies += $presetLayers.Strategies
        } else {
    ...
    return Compose-TemplateLayers -LayerPaths $layerPaths -LayerStrategies $layerStrategies
}
