  ...
  useEffect(() => {
    const fetchFeedback = async () => {
      try {
        ...
        const res = await axios.get(`http://localhost:8080/api/interviews/${id}/feedback`, {
        ...
        if (res.status === 204) {
          ...
          const generateRes = await axios.post(`http://localhost:8080/api/interviews/${id}/feedback/generate`, null, {
            headers: { Authorization: `Bearer ${token}` }
          });
